...
# Detta skript konverterar PNG-bilder till WebP-format för bättre prestanda
...
# Hitta alla bilder som behöver konverteras
$imageFiles = @()
foreach ($format in $supportedFormats) {
    $files = Get-ChildItem -Path $inputDir -Filter $format
    $imageFiles += $files
}

if ($imageFiles.Count -eq 0) {
    Write-Host "Inga bilder hittades att konvertera i $inputDir" -ForegroundColor Yellow
    exit 0
}

Write-Host "Hittade $($imageFiles.Count) bilder att konvertera:" -ForegroundColor Cyan
foreach ($file in $imageFiles) {
    $webpPath = Join-Path $inputDir ($file.BaseName + ".webp")
    if (Test-Path $webpPath) {
        Write-Host "  - $($file.Name) (WebP finns redan)" -ForegroundColor Gray
    } else {
...
foreach ($file in $imageFiles) {
    ...
    $outputPath = Join-Path $inputDir ($file.BaseName + ".webp")
...
Write-Host "Totalt: $($imageFiles.Count)" -ForegroundColor White

if ($successCount -gt 0) {
    Write-Host "`nWebP-filer skapade i: $inputDir" -ForegroundColor Green
}
